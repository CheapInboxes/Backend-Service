...
export async function adminRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{
    ...
    async (request, reply) => {
      try {
        // Get subscriptions with their items
        ...
        const message = error instanceof Error ? error.message : 'Unknown error';
        return reply.code(400).send({ error: { code: 'SUBSCRIPTIONS_FETCH_FAILED', message } });
      }
    }
  );
    ...
    async (request, reply) => {
      try {
        ...
        if (existingUser) {
          ...
          // Create user in Supabase Auth (they'll need to set password via magic link)
          const { data: authUser, error: authError } = await supabase.auth.admin.createUser({
            email,
            email_confirm: false,
          });
        ...
        const message = error instanceof Error ? error.message : 'Unknown error';
        return reply.code(400).send({ error: { code: 'INVITE_FAILED', message } });
      }
    }
  );
  ...
  fastify.patch<{
    ...
    async (request, reply) => {
      try {
        ...
        const message = error instanceof Error ? error.message : 'Unknown error';
        return reply.code(400).send({ error: { code: 'UPDATE_FAILED', message } });
      }
    }
  );
  ...
  fastify.delete<{
    ...
    async (request, reply) => {
      try {
        ...
        const message = error instanceof Error ? error.message : 'Unknown error';
        return reply.code(400).send({ error: { code: 'REMOVE_FAILED', message } });
      }
    }
  );
}
